use crate::{point::{Point3}, ray::{Ray, HitRecord, Hittable}, material::Material};
...
impl Hittable for Sphere {
    fn hit(&self, ray: &Ray, t_min: f64, t_max: f64) -> Option<(HitRecord, &Box<dyn Material>)> {
        ...
        if discriminant < 0.0 {
            ...
            return Some((self.build_hit_record(ray, root), &self.material))
        }
    }
}
...
mod tests {
    ...
    fn test_hit_sphere_in_center() {
        ...
        let (hit, _scatter) = sphere.hit(&ray, 0.0, 100.0).unwrap();
